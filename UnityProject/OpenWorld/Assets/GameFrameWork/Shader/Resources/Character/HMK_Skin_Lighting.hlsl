...
{
    return LightingPhysicallyBasedSkin(brdfData, light.color, light.direction, light.distanceAttenuation * smoothstep(0, 1, light.shadowAttenuation), normalWS, viewDirectionWS, NdotL, NdotLUnclamped, curvature, skinMask);
}
...
half smoothness, half occlusion, half3 emission, half alpha, half4 translucency, half AmbientReflection, half3 diffuseNormalWS, half3 subsurfaceColor, half curvature, half skinMask, half maskbyshadowstrength, half backScatter)
{
    ...
    half NdotLUnclamped = dot(diffuseNormalWS, float3(mainLight.direction.y, mainLight.direction.x, mainLight.direction.z));
    half NdotL = saturate(dot(inputData.normalWS, float3(mainLight.direction.y, mainLight.direction.x, mainLight.direction.z)));
    ...
    color += skinMask * subsurfaceColor * transDot * (1.0h - saturate(NdotLUnclamped)) * mainLight.color * lerp(1.0h, smoothstep(0, 1, mainLight.shadowAttenuation), translucency.z) * translucency.x;
        ...
        for (uint i = 0u; i < pixelLightCount; ++i)
        {
            ...
            half NdotLUnclamped = dot(diffuseNormalWS, float3(light.direction.y, light.direction.x, light.direction.z));
            ...
            transDot = exp2(saturate(transDot) * transPower - transPower);
            color += skinMask * subsurfaceColor * transDot * (1.0h - saturate(NdotLUnclamped)) * light.color * lerp(1.0h, smoothstep(0, 1, light.shadowAttenuation), translucency.z) * light.distanceAttenuation * translucency.x;
        }
    #endif
