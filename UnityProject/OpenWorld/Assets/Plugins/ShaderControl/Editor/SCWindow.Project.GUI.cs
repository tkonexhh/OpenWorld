...
namespace ShaderControl {
    ...
    public partial class SCWindow : EditorWindow {
        ...
        bool hideReadOnlyShaders;
        string projectShaderNameFilter;
        string projectShaderPathFilter;
        ...
        void DrawProjectGUI() {
            ...
            if (shaderCount > 0) {
                ...
                if (sortType != SortType.Keyword) {
                    ...
                    projectShaderPathFilter = EditorGUILayout.TextField(projectShaderPathFilter);
                    if (GUILayout.Button(new GUIContent("Clear", "Clear filter."), EditorStyles.miniButton, GUILayout.Width(60))) {
                        projectShaderPathFilter = "";
                        GUIUtility.keyboardControl = 0;
                    }
                    EditorGUILayout.EndHorizontal();
                ...
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField("Keyword Name", GUILayout.Width(100));
                ...
                if (sortType != SortType.Material) {
                    ...
                    if (sortType != SortType.Material) {
                        hideReadOnlyShaders = EditorGUILayout.ToggleLeft("Hide read-only/internal shaders", hideReadOnlyShaders);
                    }

                    if (keywordScopeFilter != KeywordScopeFilter.GlobalKeywords || pragmaTypeFilter != PragmaTypeFilter.ShaderFeature) {
                        if (totalGlobalShaderFeaturesNonReadonly > 0) {
                            if (GUILayout.Button(new GUIContent("Click to show " + totalGlobalShaderFeaturesNonReadonly + " global keyword(s) found of type 'shader_feature'", "Lists all shaders using global keywords that can be converted to local safely."))) {
                                ...
                                keywordFilter = "";
                                hideReadOnlyShaders = true;
                                GUIUtility.keyboardControl = 0;
                                minimumKeywordCount = 0;
                            }
                        }
                    } else {
                        if (sortType == SortType.Keyword && totalGlobalShaderFeaturesNonReadonly > 1) {
                            if (GUILayout.Button(new GUIContent("Convert " + totalGlobalShaderFeaturesNonReadonly + " keyword(s) to local", "Converts all global keywords of type Shader_Feature to local keywords, reducing the total keyword usage count.\n A local keyword does not count towards the limit of 256 total keywords in project."))) {
                                if (EditorUtility.DisplayDialog("Convert " + totalGlobalShaderFeaturesNonReadonly + " global keyword(s) to local", "The keywords will be converted to local. This means these keywords won't count toward the maximum global keyword limit (256). Continue?", "Ok", "Cancel")) {
                                    ConvertToLocalAll();
                                    EditorUtility.DisplayDialog("Process complete!", "Please restart Unity to refresh keyword changes.", "Ok");
                                    ScanProject();
                                    GUIUtility.ExitGUI();
                                }
                            }
            ...
            bool usesShaderPathFilter = !string.IsNullOrEmpty(projectShaderPathFilter);
            ...
            if (sortType == SortType.Material) {
                if (projectMaterials != null) {
                    ...
                    for (int s = 0; s < matCount; s++) {
                        ...
                        if (usesShaderNameFilter && mat.unityMaterial.name.IndexOf(projectShaderNameFilter, StringComparison.InvariantCultureIgnoreCase) < 0)
                            continue;

                        if (needsTitle) {
                            needsTitle = false;
                            GUILayout.Label("Materials found:");
                        }
                ...
                if (keywordView != null) {
                    ...
                    for (int s = 0; s < kvCount; s++) {
                        ...
                        SCKeyword keyword = kv.keyword;
                        if (hideReadOnlyShaders && !keyword.canBeModified) continue;
                        ...
                        if (showKeywordsNotUsedInBuild && totalBuildShaders > 0) {
                            ...
                            for (int sb = 0; sb < totalBuildShaders; sb++) {
                                ...
                                int count = sbi.keywords != null ? sbi.keywords.Count : 0;
                        ...
                        if (needsTitle) {
                            needsTitle = false;
                            GUILayout.Label("Keywords found:");
                        }
                        ...
                        sb.Append(rowNumber++);
                        sb.Append(": <b>");
                        sb.Append(kv.keyword);
                        sb.Append("</b> found in ");
                        sb.Append(kvShadersCount);
                        if (!keyword.canBeConvertedToLocal) {
                            sb.Append(" read-only");
                        }
                        sb.Append(" shader(s)");

                        kv.foldout = EditorGUILayout.Foldout(kv.foldout, new GUIContent(sb.ToString()), foldoutRTF);
                        GUILayout.FlexibleSpace();
                        if (keyword.canBeConvertedToLocal) {
                        ...
                        if (kv.foldout) {
                            for (int m = 0; m < kvShadersCount; m++) {
                                ...
                                if (hideReadOnlyShaders && shader.isReadOnly) continue;
                                ...
                                if (!shader.hasBackup)
                                    GUI.enabled = false;
                                if (GUILayout.Button(new GUIContent("Restore", "Restores shader from the backup copy."), EditorStyles.miniButton, GUILayout.Width(70))) {
                ...
                int bkvCount = !hideReadOnlyShaders && !showKeywordsNotUsedInBuild && keywordViewExtra != null ? keywordViewExtra.Count : 0;
                if (bkvCount > 0) {
                    needsTitle = true;
                    EditorGUILayout.Separator();
                    bool usesKeywordFilter = !string.IsNullOrEmpty(keywordFilter);
                    int rowNumber = 1;
                    for (int s = 0; s < bkvCount; s++) {
                        BuildKeywordView bkv = keywordViewExtra[s];
                        string keywordName = bkv.keyword;
                        if (usesKeywordFilter && keywordName.IndexOf(keywordFilter, StringComparison.InvariantCultureIgnoreCase) < 0)
                            continue;
                        if (needsTitle) {
                            needsTitle = false;
                            GUILayout.Label("Additional keywords used in last build:");
                        }
                        EditorGUILayout.BeginHorizontal();
                        keywordViewExtra[s].foldout = EditorGUILayout.Foldout(bkv.foldout, new GUIContent("Keyword " + (rowNumber++) + ": <b>" + keywordName + "</b> found in " + bkv.shaders.Count + " shader(s)"), foldoutRTF);
                        EditorGUILayout.EndHorizontal();
                        if (bkv.foldout) {
                            int kvShadersCount = bkv.shaders.Count;
                            for (int m = 0; m < kvShadersCount; m++) {
                                EditorGUILayout.BeginHorizontal();
                                EditorGUILayout.LabelField("", GUILayout.Width(30));
                                EditorGUILayout.LabelField(shaderIcon, GUILayout.Width(18));
                                ShaderBuildInfo shader = bkv.shaders[m];
                                string shaderName = shader.name;
                                if (shader.isInternal) shaderName += " (internal)";
                                GUIStyle shaderNameStyle = shader.isInternal ? labelDim : labelNormal;
                                EditorGUILayout.LabelField(shaderName, shaderNameStyle);
                                EditorGUILayout.EndHorizontal();
                            }
                        }
                ...
                for (int s = 0; s < shaderCount; s++) {
                    ...
                    if (usesShaderPathFilter && shader.path.IndexOf(projectShaderPathFilter, StringComparison.InvariantCultureIgnoreCase) < 0) {
                        continue;
                    }
                ...
                if (shadersListedCount == 0) {
                    if (usesShaderNameFilter && usesShaderPathFilter) {
                        EditorGUILayout.HelpBox("No shader matching '" + projectShaderNameFilter + "' name in '" + projectShaderPathFilter + "' path found. Either the shader doesn't exist in the path as source file or the shader does not use explicit keywords.", MessageType.Info);
                    } else if (usesShaderNameFilter) {
                        ...
                        EditorGUILayout.HelpBox("No shader matching criteria found in '" + projectShaderPathFilter + "' path found. Either the shader doesn't exist in the path as source file or the shader does not use explicit keywords.", MessageType.Info);
                    }
                }
            }
